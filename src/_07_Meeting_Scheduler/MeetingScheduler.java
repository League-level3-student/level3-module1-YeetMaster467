...
public class MeetingScheduler {
	...
	public static Schedule getMutualAvailability(Schedule person1, Schedule person2) {
		...
		HashMap<String, ArrayList<Integer>> mutualAvailibility = new HashMap<String, ArrayList<Integer>>();
		ArrayList<Integer> person2mutualTimes = null;
		ArrayList<Integer> person1mutualTimes = null;
		Schedule s = new Schedule();

		for (String key : person1.getSchedule().keySet()) {
			person2mutualTimes = person2.getSchedule().get(key);
			person1mutualTimes = person1.getSchedule().get(key);
			
			for (Integer mutualTimes: person1mutualTimes) {
				for (Integer time: person2mutualTimes) {
					if (mutualTimes == time) {
						s.addAvailability(key, time);
					}
				}
			}
		}
